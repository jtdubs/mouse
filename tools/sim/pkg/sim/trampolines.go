...
int on_command(struct avr_t *avr, uint8_t v, void *param);
...
__attribute__((weak))
int on_command_cgo(struct avr_t *avr, uint8_t v, void *param) {
	return on_command(avr, v, param);
}
*/
import "C"
...
type CommandHandler interface {
	OnCommand(avr *C.avr_t, v uint8, param unsafe.Pointer) C.int
}
...
var (
	...
	on_command_cgo  = C.avr_cmd_handler_t(C.on_command_cgo)
)
...
//export on_command
func on_command(avr *C.avr_t, v uint8, param unsafe.Pointer) C.int {
	return pointer.Restore(param).(CommandHandler).OnCommand(avr, v, param)
}
